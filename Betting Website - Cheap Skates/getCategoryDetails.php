...
$currentTime = date('Y-m-d H:i:s', $currentTimeUnix);
  ...
  $getCatKey = mysql_query("SELECT * FROM csEvents WHERE CatKey='$categoryID' AND EndDandT > '$currentTime' ORDER BY StartDandT ASC");  
  $getCatName = mysql_query("SELECT * FROM csCategories WHERE CatKey='$categoryID'");
  $catName = mysql_fetch_array($getCatName);
