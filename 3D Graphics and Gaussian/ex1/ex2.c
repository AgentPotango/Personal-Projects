...
void calculate_lookpoint(void) { /* Given an eyepoint and latitude and longitude angles, will
	...
	double dirx = cos(lat * DEG_TO_RAD) * sin(lon * DEG_TO_RAD)  ;
	double diry = sin(lat * DEG_TO_RAD) ;
	double dirz = cos(lat * DEG_TO_RAD) * cos(lon * DEG_TO_RAD);
...
void mouse_motion(int x, int y) {
  ...
  /* To be completed */
   ...
   //mlon = 50 * distancex/(float)
   //mlat = 50 * distancey/(float)
   mlon = 50 +  (100 * x)/width;
