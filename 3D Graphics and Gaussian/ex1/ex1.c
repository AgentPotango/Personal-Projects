...
void calculate_lookpoint(void) { /* Given an eyepoint and latitude and longitude angles, will
  ...
  double totalLat, totalLon;
	
	totalLat = mlat + lat;
	totalLon = mlon + lon;
	
  
  dirx = cos(totalLat * DEG_TO_RAD) * sin(totalLon * DEG_TO_RAD)  ;
	diry = sin(totalLat * DEG_TO_RAD) ;
	dirz = cos(totalLat * DEG_TO_RAD) * cos(totalLon * DEG_TO_RAD);
...
void mouse_motion(int x, int y) {
  ...
  /* To be completed */
   ...
   double currentLat = mlat + lat;
   
   mlon = 50 -  (100 * x)/width;
   mlat = 50 - (100 * y)/height;
  
  
	if (mlat + lat >= 90)
	  mlat = currentLat - lat - 0.5;
  else if (mlat + lat <= -90)
   	mlat = currentLat - lat + 0.1;
