  ...
  if((fp=fopen(infile, "rb")))
    {
      ...
      while((!feof(fp))&&(c<N))
	{
	  fscanf(fp, "%lf\t%lf\t%lf\t%d\t%d\n", &list[c].rating,  &list[c].price, &list[c].relevance, &list[c].pageNumbers, &list[c].ID);	  
	  c++;
	}
      fclose(fp);      
    }
...
void user_interface(int N)
{
	...
	if (choice == 1)
  {
    ...
    scanf("%d", &noOfFields);
		...
		if (noOfFields != 1 && noOfFields !=2 && noOfFields!= 3 && noOfFields!= 4)
		{
...
void print_results(int N)
{
  ...
  if((fp=fopen("top20.txt","w")))
      ...
      for(i=N-1;i>=N-20;i--)
	{
	  ...
	  printf("%g %g %g %d %d\n", list[i].rating, list[i].price, list[i].relevance, list[i].pageNumbers, list[i].ID);
