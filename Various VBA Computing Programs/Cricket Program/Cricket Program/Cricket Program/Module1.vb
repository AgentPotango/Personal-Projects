Module Module1
    ...
    Sub Main()
        ...
        Do
            ...
            Loop Until (OptionSelected >= 1 And OptionSelected <= 7) Or OptionSelected = 9
            Console.WriteLine()
            If OptionSelected >= 1 And OptionSelected <= 7 Then
                Select Case OptionSelected
                    ...
                    Case 6 : FindHighestScore(TopScores)
                    Case 7 : SortHighScores(TopScores)
                End Select
            End If
    ...
    Function GetValidPlayerName() As String
        ...
        Do
            ...
            If IsNumeric(PlayerName) Or PlayerName.Length = 0 Then
                Console.Write("That is not a valid name.  Please try again: ")
            End If
        Loop Until PlayerName <> "" And Not IsNumeric(PlayerName)
    ...
    Sub DisplayMenu()
        ...
        Console.WriteLine("6.  Locate Highest Score")
        Console.WriteLine("7.  Sort High Scores")
    ...
    Function GetMenuChoice() As Integer
        ...
        If IsNumeric(TempOption) Then
            OptionChosen = TempOption
        Else
            OptionChosen = 0
        End If
        If OptionChosen < "1" Or (OptionChosen > "7" And OptionChosen <> "9") Then
            Console.WriteLine()
    ...
    Sub DisplayCurrentPlayerNewScore(ByVal CurrentPlayerScore As Integer)
        If CurrentPlayerScore = 100 Then
            Console.WriteLine("Your new score is: A Century!")
        ElseIf CurrentPlayerScore = 50 Then
            Console.WriteLine("Your new score is: A Half-Century!")
        Else
    ...
    Function RollAppealDie(ByVal VirtualDiceGame As Boolean) As Integer
        ...
        Else
            ...
            Console.WriteLine("Enter 5 if the result is STUMPED")
            Console.WriteLine("Enter 6 if the result is RUNOUT")
            Console.WriteLine()
            Console.Write("Result: ")
            Do
                AppealDieResult = Console.ReadLine
                Console.WriteLine()
            Loop Until AppealDieResult >= 1 And AppealDieResult <= 6

        End If
        RollAppealDie = AppealDieResult
    End Function

    Sub DisplayAppealDieResult(ByVal AppealDieResult As Integer, ByVal CScore As Integer)
        Select Case AppealDieResult
            ...
            Case 6
                Console.WriteLine("RunOut")
                If CScore = 0 And AppealDieResult >= 1 And AppealDieResult <= 6 Then
                    Console.WriteLine("Out For A Duck!")
                End If
        End Select
    End Sub
  ...
  ByVal PlayerTwoName As String, ByVal PlayerTwoScore As Integer)
        ...
        End If
        If PlayerOneScore = PlayerTwoScore Then
            Console.WriteLine("The Match was Drawn")
        End If
        Console.WriteLine()
    End Sub
  ...
  ByVal PlayerScore As Integer)
    ...
    Sub SaveTopScores(ByRef TopScores() As TTopScore)
        ...
        For C = 1 To MaxSize
            LineToFile = TopScores(C).Name & "," & TopScores(C).Score
            PrintLine(1, LineToFile)
        Next
        Console.WriteLine("The Scores Have Been Saved")
        FileClose(1)
    End Sub
  ...
  ByVal VirtualDiceGame As Boolean, ByRef TopScores() As TTopScore)
        ...
        For PlayerNo = 1 To 2
            ...
            Do
                ...
                If BowlDieResult >= 1 And BowlDieResult <= 4 Then
                    ...
                    DisplayCurrentPlayerNewScore(CurrentPlayerScore)
                    'Console.WriteLine("Your new score is: " & CurrentPlayerScore)
                End If
                ...
                If BowlDieResult = 6 Then
                    ...
                    DisplayAppealDieResult(AppealDieResult, CurrentPlayerScore)
    ...
    Private Sub FindHighestScore(ByRef TopScore() As TTopScore)
        Dim L As Integer ' Location of TopScore
        Dim HS As Integer ' Value of TopScore
        Dim HSName As String ' Name of Top Scorer
        Dim NoHs As Integer ' Amount of HighScores
        HS = TopScore(1).Score
        HSName = TopScore(1).Name
        L = 1
        NoHs = TopScore.Length - 1

        For C = 2 To NoHs
            If TopScore(C).Score > HS Then
                L = C
                HS = TopScore(C).Score
                HSName = TopScore(C).Name
            End If
        Next

        If HSName = "-" Then
            Console.WriteLine("There are currently no TopScores")
        Else
            Console.WriteLine("The highest score was " & HS)
            Console.WriteLine("It was scored by " & HSName)
        End If
    ...
    Private Sub SortHighScores(ByRef TopScores() As TTopScore)
        Dim NoHs As Integer 'Number of HighScores
        Dim Swap As Boolean
        Dim Temp As TTopScore
        NoHs = TopScores.Length - 1

        Do
            Swap = False

            For C = 1 To NoHs - 1
                If TopScores(C).Score < TopScores(C + 1).Score Then
                    Swap = True
                    Temp = TopScores(C)
            ...
            Next
            NoHs = NoHs - 1
        Loop Until (Not Swap Or NoHs = 1)
        Console.WriteLine("HighScores Sorted!")
        Console.WriteLine()
        Console.WriteLine("Press the Enter key to continue")
        Console.ReadLine()
    End Sub
